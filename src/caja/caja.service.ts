...
import { Caja, CajaDocument } from './schema/schema.caja';
...
export class CajaService {
    ...
    constructor(
        @InjectModel(Caja.name) private CajaModule:Model<CajaDocument> ,
    
           // @InjectRepository(Enterprises)
           // private EnterpriseModule :Repository<Enterprises>
        ){}
    
        async get():Promise<Caja[]|HttpException>{
            try {
                const res= await this.CajaModule.find();
        ...
        async getId(id:ObjectId):Promise<Caja|HttpException>{
            try {
                
                const found=await this.CajaModule.findOne({_id:id,estado:'A'})
